function deleteOwnFile(a){var b=confirm("Are you sure that you want to delete that file?");if(b==true){$.get("reference.php",{f:a,func:"delete_file",},function(c){$("#center").empty();
...
var d=document.user_pref.twitter_id.value;var e=document.user_pref.facebook_id.value;if(IsEmpty(c)==true&&validateEmail(b)){$.get("reference.php",{func:"user_pref",real_name:c,email:b,show_real_name:f,show_email:a,twitter_id:d,facebook_id:e,},function(g){if(g=="true"){$(".upError").fadeOut();
...
var t=x[0]["f_size"];var z=x[0]["f_file"];$("#header").html(z);var y=document.createElement("table");y.setAttribute("id","table");var r=document.createElement("th");
r.innerHTML="Uploaded by";var q=document.createElement("th");q.innerHTML="Size";var p=document.createElement("th");p.innerHTML="Date uploaded";var o=document.createElement("th");
...
td4=document.createElement("td");td4.innerHTML=j;if(j.substring(0,6)=="image/"){var d=document.createElement("td");var A=new Image();A.src="./printimage.php?id="+e;
A.onload=function(){width=this.width;height=this.height;d.innerHTML="Height: "+height+"Width: "+width;};}var c=document.createElement("td");var u=document.createElement("a");
u.setAttribute("href","?page=download_analysis&file="+a);$.get("reference.php",{func:"fileinfo",action:"times_downloaded",file:a,},function(B){u.innerHTML=B;
